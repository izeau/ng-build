...
function ngProvider(options) {
    ...
    function _ngProvider(file, enc, done) {
        ...
        file.data = extend(file.data, {
            ...
            name: name,
            type: type,
            contents: replaceTokens(file.contents.toString(), options)
        });
...
function replaceTokens(content, options) {
    ...
    for (var key in options.tokens) {
        reg = new RegExp(options.tokenSep + key + options.tokenSep , 'gi');

        content = content.replace(reg, options.tokens[key]);
    }
